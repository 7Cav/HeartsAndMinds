...
private _cities = ["NameVillage", "NameCity", "NameCityCapital", "NameLocal", "Airport", "BorderCrossing", "VegetationFir"];
...
for "_id" from 0 to (count _locations - 1) do {
    ...
    if (_type in _cities) then {
        ...
        if (_position distance getMarkerPos "SAFE_AREA" < 2000) exitWith {};
